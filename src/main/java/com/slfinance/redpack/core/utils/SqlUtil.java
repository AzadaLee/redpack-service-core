...
public class SqlUtil {
	...
	public static final String SELECT_COUNT_FROM = SELECT_COUNT + BLANK + FROM;
	...
	public static String getTotalSql(String sql) {
		return SELECT_COUNT_FROM + " ( " + sql + " ) temp_";
	}
	...
	public static String getPageSql(String sql, int start, int length) {
		return "SELECT * FROM ( SELECT ROWNUM as rowno,temp_.*  FROM (" + sql + ") temp_ ) temp__ WHERE temp__.rowno>" + start + " AND temp__.rowno<=" + (start + length);
	}
}
