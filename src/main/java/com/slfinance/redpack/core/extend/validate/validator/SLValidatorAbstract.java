...
public abstract class SLValidatorAbstract implements SLValidator {
	...
	protected SLValidatorAbstract appVersion(String key, boolean check, String message) {
		if (check) {
			populate(key, RuleUtils.isAppVersion(getValue(key)), formatMessage(message));
		}
		return this;
	}
}
