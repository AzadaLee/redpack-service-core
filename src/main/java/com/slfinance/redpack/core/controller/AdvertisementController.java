...
public class AdvertisementController extends BaseController {
	...
	public ResponseVo appEditInfo(@RequestBody Map<String, Object> params) {
		...
		if (advertisement != null) {
			...
			result.put("content", advertisement.getContent());
			result.put("hyperlink", advertisement.getHyperlink());
			List<Map<String, Object>> filesList = new ArrayList<Map<String, Object>>();
			List<FileRelation> FileRelationLsit = fileRelationService.findByAdvertisementAndRelatePrimary(advertisement.getId());
			for (FileRelation fileRelation : FileRelationLsit) {
				Map<String, Object> file = new HashMap<String, Object>();
	...
	public ResponseVo appList(PageRequestVo pageRequest) throws SLException {
		...
		pageRequest.addParam("status", AdvertisementStatus.未下架.toString());
		pageRequest.addParam("recordType", RecordType.正本.toString());
	...
	public ResponseVo appSelectList(){
		Map<String,Object> result = new HashMap<String,Object>();
		result.put("data", advertisementService.advertisementSelectList(getLoginUserId(),AdvertisementStatus.未下架,RecordType.正本));
		return ResponseVo.success(result);
	}
	...
	@RequestMapping("/save")
	@Rules({@Rule(name = "title", required = true, requiredMessage = "300009") ,@Rule(name = "logo", required = true, requiredMessage = "300010") ,@Rule(name = "content", required = true, requiredMessage = "300011")})
	public ResponseVo appSave(@RequestBody Map<String, Object> params) {
		...
		if (files == null || answers == null) {
			throw new SLException("300012", "附件或答案不能为空");
		}
		...
		advertisement.setUserType(UserType.客户);
