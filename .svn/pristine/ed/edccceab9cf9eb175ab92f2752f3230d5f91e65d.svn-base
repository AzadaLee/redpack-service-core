...
public class MessageService extends BaseService<Message, MessageRepository> {
	...
	public PageResponse<Map<String, Object>> findMessagesListByUserId(PageRequestVo pageRequest) {
		// 先查询出客户要显示的纪录
		PageResponse<Map<String, Object>> result = repository.findMessagesListByUserId(pageRequest);
		// 把未读的信息设置成已读
		List<Message> listNotRedMessage = repository.findByCustomerIdAndIsRead(pageRequest.getParam("customerId").toString(), 0);
		for (Message message : listNotRedMessage) {
			message.setIsRead(1);
	...
	public Message sendMessage(MessageTemplateMessageType messageType, String customerId, String relatePrimary) {
		...
		if (messageType == MessageTemplateMessageType.充值成功 || messageType == MessageTemplateMessageType.充值失败) {
			...
			content = StringUtil.replaceOnce(content, MessageTemplateService.PLACEHOLDER_PREFIX + "month" + MessageTemplateService.PLACEHOLDER_SUFFIX, (c.get(Calendar.MONTH)+1) + "");
