...
public class AccountService extends BaseService<Account, AccountRepository> {
	 ...
	 */
	@Transactional
	public void saveAppRedPackUnFreezeFlow(Account account,RedPack redpack,AccountFlow redPackFreezeFlow){
		AccountFlow redPackUnFreezeFlow = new AccountFlow(account.getId(), codeGeneratorService.getAccountFlowCode(), AccountFlowflowType.红包解冻, redPackFreezeFlow.getTradeAmount(), AccountFlowTradeDirection.解冻, redpack.getId(), redPackFreezeFlow.getFlowCode(), "红包解冻");
	 ...
	 */
	@Transactional
	public void saveAppSendRedPackFlow(Account account,RedPack redpack,OrderDetail redPackOrderDetai,OrderDetail redPackPoundageOrderDetai){
		List<AccountFlow> accountFlows = Lists.newArrayList();
		Advertisement advertisement = advertisementService.findOne(redpack.getAdvertisementId());
		AccountFlow sendRedPackFlow = new AccountFlow(account.getId(), codeGeneratorService.getAccountFlowCode(), AccountFlowflowType.发红包, redPackOrderDetai.getAmount(), AccountFlowTradeDirection.支出, redpack.getId(), null, "发放红包\""+ StringUtil.cutAndPadding(advertisement.getTitle(), 10, "...", "...") + "\"");
		settle(account, sendRedPackFlow);
		accountFlows.add(sendRedPackFlow);
		//可能会免除手续费
		if(redPackPoundageOrderDetai != null){
			AccountFlow packPoundageFlow = new AccountFlow(account.getId(), codeGeneratorService.getAccountFlowCode(), AccountFlowflowType.发红包手续费, (redPackPoundageOrderDetai.getAmount()), AccountFlowTradeDirection.支出, redpack.getId(), null, "发放红包手续费");
