			...
			try {
				...
				if (redpack != null) {
					if (redpack.getCreatedDate().getTime() < systemTime.getTime()) {
						// 订单生成25分钟还没有支付
						if (DateUtil.addMinutes(redpack.getCreatedDate(), 25).compareTo(systemTime) <= 0) {
							// 发送站内信
							messageService.sendMessage(MessageTemplateMessageType.未付款红包即将到期, redpack.getCreatedUser(), redpack.getId());
						} else {
							// 休眠
							Thread.sleep(DateUtil.addMinutes(redpack.getCreatedDate(), 25).getTime() - systemTime.getTime());
						}
					} else {
						// 订单失效
						messageService.sendMessage(MessageTemplateMessageType.未付款红包即将到期, redpack.getCreatedUser(), redpack.getId());
